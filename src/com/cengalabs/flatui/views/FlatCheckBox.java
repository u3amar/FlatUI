...
public class FlatCheckBox extends CheckBox implements Attributes.AttributeChangeListener
{
	...
	private void init(AttributeSet attrs)
	{
		...
		if (attrs != null)
		{
			...
			attributes.setSize(a.getDimensionPixelSize(R.styleable.FlatCheckBox_size, Attributes.globalSize));
			attributes.setRadius(a.getDimensionPixelSize(R.styleable.FlatCheckBox_cornerRadius, Attributes.globalRadius));
			attributes.setBorderWidth(a.getDimensionPixelSize(R.styleable.FlatCheckBox_size, Attributes.globalSize) / 10);
		...
		InsetDrawable checkedInside = new InsetDrawable(checkedCore, attributes.getBorderWidth() + dotMargin,
				attributes.getBorderWidth() + dotMargin, attributes.getBorderWidth() + dotMargin, attributes.getBorderWidth() + dotMargin);

		Drawable[] checkedEnabledDrawable =
		{
				checkedOutside, checkedInside
		};
		...
		InsetDrawable checkedInsideDisabled = new InsetDrawable(checkedCoreDisabled, attributes.getBorderWidth() + dotMargin,
				attributes.getBorderWidth() + dotMargin, attributes.getBorderWidth() + dotMargin, attributes.getBorderWidth() + dotMargin);

		Drawable[] checkedDisabledDrawable =
		{
				checkedOutsideDisabled, checkedInsideDisabled
		};
