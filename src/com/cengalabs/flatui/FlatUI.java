...
public class FlatUI
{
	...
	private static ArrayList<Attributes> attachedElements = new ArrayList<Attributes>();
	...
	{
		Attributes.globalBorderWidth = (int) dipToPx(context, Attributes.DEFAULT_BORDER_WIDTH);
		Attributes.globalRadius = (int) dipToPx(context, Attributes.DEFAULT_RADIUS);
		Attributes.globalSize = (int) dipToPx(context, Attributes.DEFAULT_SIZE);
	}
	...
	public static Typeface getFont(Context context, Attributes attributes)
	{
		...
		catch (Exception e)
		{
			Log.e("FlatUI", "Font file at " + fontPath + " cannot be found or the file is "
					...
					+ "to project. If not, copy assets/fonts folder of the library to your " + "projects assets folder.");
			return null;
		}
	}
	 ...
	 *            selected theme
	 * @param dark
	 *            boolean for choosing dark colors or primary colors
	 * @param borderBottom
	 *            bottom border width
	...
	/**
	 * Sets the default theme of the application. The views which doesn't have any theme attribute will have this defined default theme.
	 * <p/>
	 * IMPORTANT: This method should be called before setContentView method of the activity.
	 * 
	 * @param theme
	 */
	public static void setDefaultTheme(int theme, Context context)
	{
		Attributes.DEFAULT_THEME = theme;
		
		for (Attributes attributes : attachedElements)
		{
			attributes.setTheme(theme, context.getResources());
		}
	}
	
	public static void addFlatUIElement(Attributes attributesForElement)
	{
		if (!attachedElements.contains(attributesForElement))
		{
			attachedElements.add(attributesForElement);
		}
	}
